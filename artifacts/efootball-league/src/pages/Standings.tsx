...
import { exportStandingsCSV, printStandings } from "@/lib/export";
      ...
      {leagues.length === 0 ? (
        <Card className="border-dashed bg-muted/20">
          <CardContent className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">You are not a member of any leagues yet.</p>
          </CardContent>
        </Card>
      ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="gap-1.5 text-xs h-8"
                      onClick={() => exportStandingsCSV(league.name, data.teams, data.matches)}>
                      <Download className="h-3.5 w-3.5" />CSV
                    </Button>
                    <Button variant="outline" size="sm" className="gap-1.5 text-xs h-8"
                      onClick={() => printStandings(league.name, data.teams, data.matches)}>
